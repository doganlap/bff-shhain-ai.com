...
router.get('/', authenticateToken, requireTenantAccess, async (req, res) => {
  try {
    ...
    if (framework_id) {
      paramCount++;
      whereClause += ` AND at.framework_id = $${paramCount}`;
      params.push(framework_id);
    }

    if (category) {
      paramCount++;
      whereClause += ` AND at.category = $${paramCount}`;
      params.push(category);
    }

    if (search) {
      paramCount++;
      whereClause += ` AND (at.name ILIKE $${paramCount} OR at.description ILIKE $${paramCount})`;
      params.push(`%${search}%`);
    }
