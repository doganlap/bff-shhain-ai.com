...
if (-not $allHealthy) {
    Write-Host "Some services are not running properly. Please check Docker logs." -ForegroundColor Red
    exit 1
}

Write-Host "Setting up database permissions..." -ForegroundColor Yellow
...
Write-Host "3. Test health: curl http://localhost:3005/healthz" -ForegroundColor White
